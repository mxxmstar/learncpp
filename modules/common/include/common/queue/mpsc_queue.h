 ...
 *
 * 提供两种实现�? *   - BoundedMpscQueue<T>�? 有界（boost::lockfree::queue 包装，固定节点池），
 *                            满则 push 返回 false
...
public:
    /// @brief 构造函�?    /// @param capacity 队列容量（节点池大小�?    explicit BoundedMpscQueue(size_t capacity)
        : queue_(capacity), size_(0) {}

    /// @brief 推入元素（多生产者安全）
    /// @param item 元素
    /// @return true 成功，false 队列�?    bool push(const T& item) {
    ...
    /// @brief 弹出元素（仅消费者线程调用）
    /// @param item 输出参数
    /// @return true 成功，false 队列�?    bool pop(T& item) {
...
// 基于 boost::lockfree::queue + fixed_sized<false>�?// 节点池耗尽时自动动态分配，永不拒绝
// 线程安全：多生产�?+ 单消费�?// ============================================================================
template<typename T>
class UnboundedMpscQueue {
public:
    /// @brief 构造函�?    /// @param initial_capacity 初始节点池大小（后续可动态增长）
    explicit UnboundedMpscQueue(size_t initial_capacity = 64)
        : queue_(initial_capacity), size_(0) {}

    /// @brief 推入元素（多生产者安全，永不失败�?    /// @param item 元素
