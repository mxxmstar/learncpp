...
GrpcAlgorithmProcessor::GrpcAlgorithmProcessor(const ProcessorConfig& config)
    : config_(config)
...
GrpcAlgorithmProcessor::~GrpcAlgorithmProcessor() {
    Stop();
}

bool GrpcAlgorithmProcessor::Start() {
    if (running_) {
...
void GrpcAlgorithmProcessor::Stop() {
    if (!running_) {
        return;
    }
...
bool GrpcAlgorithmProcessor::ProcessFrame(const VideoFrame& frame) {
...
void GrpcAlgorithmProcessor::SetDetectionCallback(DetectionCallback callback) {
    detection_callback_ = callback;
}

ProcessorStats GrpcAlgorithmProcessor::GetStats() const {
    std::lock_guard<std::mutex> lock(stats_mutex_);
    return stats_;
}

bool GrpcAlgorithmProcessor::IsAvailable() const {
    return running_ && grpc_client_ && grpc_client_->IsConnected();
}

// ========== 私有方法 ==========

bool GrpcAlgorithmProcessor::ShouldSendFrame() {
...
void GrpcAlgorithmProcessor::OnGrpcDetectionResult(
    ...
    for (const auto& box_data : boxes) {
        ...
        box.class_name = "object_" + std::to_string(box.class_id); // 临时名称
...
void GrpcAlgorithmProcessor::UpdateStats(int64_t processing_time_ms) {
    std::lock_guard<std::mutex> lock(stats_mutex_);
